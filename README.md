...
現在の公開版はPWA 4.1.0です。[選択肢カタログ](options.html)では、現行コードから読み込んだ全項目、初期値、画面表示名、出力語句、色チップを確認できます。全実装フェーズ、iPhone 16実機Safari確認、画像生成GPTの最終品質確認、旧・指示文生成GPTの停止と削除は完了しています。履歴と判定根拠は `docs/` 内の計画書、品質ログ、停止・削除ログを参照してください。
...
項目構造や選択肢を変更した版では、個別の移行要望がない限り旧プリセットを引き継ぎません。現在の保存形式はv4で、旧v1・v2・v3の前回値とプリセットを初回起動時に削除します。
