...
現在の公開版はPWA 6.0.0、ローカル開発候補は6.0.1です。[選択肢カタログ](options.html)では、現行コードから読み込んだ全項目、初期値、画面表示名、出力語句、色チップを確認できます。6.0.1候補では、意味を維持したまま同一主語の繰り返しを省いて指示文を短文化し、画面下部の固定操作バーを左リセット・右コピーの均等幅へ変更しました。公開状態と検証履歴は `docs/` 内の計画書、品質ログ、運用文書を参照してください。
...
項目構造や選択肢を変更した版では、個別の移行要望がない限り旧プリセットを引き継ぎません。6.0.0の保存形式はv6で、旧v1～v5の前回値とプリセットを初回起動時に削除します。
