...
現在の公開版はPWA 6.0.0です。[選択肢カタログ](options.html)では、現行コードから読み込んだ全項目、初期値、画面表示名、出力語句、色チップを確認できます。6.0.0では撮影設定を必須選択へ整理し、指示文を完全文へ統一し、関連項目のグループ表示と画面下部の固定操作バーを追加しました。公開状態と検証履歴は `docs/` 内の計画書、品質ログ、運用文書を参照してください。
