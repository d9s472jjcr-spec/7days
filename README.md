...
現在の公開版はPWA 6.1.0です。[選択肢カタログ](options.html)では、現行コードから読み込んだ全項目、初期値、画面表示名、出力語句、色チップを確認できます。6.1.0では、PWAから画風指定を分離し、撮影方向4種類を新設、ポーズを既存9種類から12種類へ拡張しました。公開状態と検証履歴は `docs/` 内の計画書、品質ログ、運用文書を参照してください。
...
項目構造や選択肢を変更した版では、個別の移行要望がない限り旧プリセットを引き継ぎません。6.1.0の保存形式はv7で、旧v1～v6の前回値とプリセットを初回起動時に削除します。
