...
現在の公開版はPWA 5.0.0です。[選択肢カタログ](options.html)では、現行コードから読み込んだ全項目、初期値、画面表示名、出力語句、色チップを確認できます。5.0.0ではアウターのレイヤー分離、衣装候補の拡充、下着・水着・靴を追加し、iPhone 16実機Safari確認まで合格しています。公開状態と検証履歴は `docs/` 内の計画書、品質ログ、運用文書を参照してください。
